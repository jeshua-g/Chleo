  ...
  commands: [
    {
      ...
      description:
        "Completely restricts access to the domain until unblocked via puzzle challenge.",
    },
    {
      syntax: "unblock <domain>",
      examples: ["unblock facebook.com"],
      description:
        "Requests to unblock access, requiring user to complete a puzzle challenge at an emotion penalty.",
    },
    {
      syntax: "limit <domain> to <number> [minutes|seconds]",
      examples: [
        "limit youtube.com to 15 minutes",
        "limit reddit.com to 30 seconds",
      ],
      description:
        "Sets daily allowed time limit on a domain. Triggers warning at threshold and blocks when reached.",
    },
    {
      syntax: "mark <domain> productive",
      examples: [
        "mark github.com productive",
        "set stackoverflow.com as productive",
      ],
      description:
        "Tags a domain as productive, granting focus streak rewards, coins, and Chleo praise.",
    },
    {
...
export function parseMonitoringCommand(text: string): ParsedCommand {
  ...
  const blockMatch = lower.match(
    /^(?:block|restrict)\s+([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})/,
  );
  if (blockMatch) {
    return {
      action: "BLOCK",
  ...
  const productiveMatch = lower.match(
  ...
  const unmarkProductiveMatch = lower.match(
